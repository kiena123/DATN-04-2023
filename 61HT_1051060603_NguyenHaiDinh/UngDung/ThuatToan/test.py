...
imageInput = np.array(Image.open("./Img/2.jpg"))
X = imageInput.reshape((imageInput.shape[0]*imageInput.shape[1], imageInput.shape[-1]))
U1 = np.array(pd.read_csv("./Img/2.csv", header = None), dtype = int)
C = 2
m = 2

# U, V = SSSFC(X, U1, C, m, 0.001)
# U, V = eSFCM(X, U1, C, m, 0.001, 150, 0.005)
U, V = eSFCM(X, U1, C, m, 0.001, 150, 1)
